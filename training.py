...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config):
    ...
    return {}, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, max_memory_allocated
...
def training_log(
    ...
    if iteration % args.log_interval == 0:
        ...
        if _REPORT_LOG and args.exit_interval and iteration % args.exit_interval == 0:
            write_csv_column('TFLOP/s/GPU',throughput)
            write_csv_column('elapsed time per iteration', elapsed_time_per_iteration * 1000.0)
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        print(f"Rank {torch.distributed.get_rank()} [Step {iteration}] nvidia-smi GPU memory: {max_memory_allocated:.2f} MB")        
        ...
        if should_exit:
            if _REPORT_LOG:
                ...
                write_csv_column('device type',"v100")
                write_csv_newline()
            break
